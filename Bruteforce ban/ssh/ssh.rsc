				...
				do={			
					...
					do={
						...
						do={
							...
							:log warning "$user, $badIP is it you? Attempt #$attemptCounter";
						}
						:if ($attemptCounter >= $attempt)\
						do={
							:log warning "ip $prevBadIP is not you! It will be banned!";
							/ip firewall address-list add list=$listName address=$badIP timeout=$timeout comment="ssh ban script ($content)";
						...
						do={
							/ip firewall address-list add list=$listName address=$badIP timeout=$timeout comment="ssh ban script ($user via $service)";
							:log warning "$badIP has been banned ($user via $service)";
						}
					}
				}
			:set prevBadIP $badIP;#for check #3
			}
		} on-error={ :log error "SSH ban script has crashed"; }
	}
:log info "SSH ban script was executed properly";